...
- Tronscan-профиль без ключа (`checkTronProfile`): `risk`-флаг из `account/list` (+вердикт, малый вес в скоринге) и поведенческие теги из `account/tag` (показ в отчёте). Проверено: `risk=false` даже на замороженных адресах — детектором фризов не является.
...
- Проверка адреса (`/app/aml`): скрининг + для TRON риск-анализ (`src/utils/kyt.js`, BFS-обход 1–5 хопов (лимиты 20/20/12/8 на 2–5-м, visited-множество, пул ×4, прогресс). Механика итога — max-источник (не сумма): каждый сигнал (санкции/фриз/фрод/скам/подделка/поведение) даёт 0–100 с затуханием по хопам, итог = сильнейший; direct/inbound/outbound считаются раздельно; доли входящего объёма по категориям; таблицы источников и контрагентов (адрес/получено/отправлено/txs/флаг); печать в PDF. Результат хранится в истории (`kyt` поле) и переиспользуется из кэша; старые записи открываются кликом с полными деталями.
